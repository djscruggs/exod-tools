...
export class ExodTools extends Contract {
  ...
  borrowStablecoin(borrowAmount: uint64, exodPrice: uint64): void {
    ...
    // Update loan data
    loanBox.value = this.encodeLoanData(collateralAmount, newBorrowedAmount, Global.latestTimestamp);
  ...
  repayLoan(payTxn: gtxn.AssetTransferTxn): void {
    ...
    // Update loan data
    loanBox.value = this.encodeLoanData(collateralAmount, borrowedAmount - payTxn.assetAmount, Global.latestTimestamp);
  }
